...
class CodeProcessor:
    ...
    def get_text_and_line_number(self, x, y):
        ...
        pyautogui.press('escape')  # Clear selection
        time.sleep(CONFIG['CLICK_DELAY'])
    ...
    def select_code_block_with_key_line(self, key_line, total_lines):
        """
        Select a code block by first finding the key line with OCR, then using mouse dragging
        to select the entire code block.

        Args:
            key_line: The line that determines the start of the code block
            total_lines: The total number of lines to select
        """
        try:
            self.log(f"Finding key line using OCR: '{key_line}'")
            ...
            self.log(f"Found key line at position ({start_x}, {start_y})")

            # Ensure we're at the beginning of the line
            self.ocr_selector.ensure_beginning_of_line(start_x, start_y)

            # Now use the mouse dragging method to select the text
            selected_text, end_y = self.ocr_selector.select_text_lines(
                start_x,
                start_y,
                total_lines,
                line_height=max(estimated_line_height, 15)  # Use at least 15px as minimum
            ...
            if not selected_text:
                self.log("Failed to select text using mouse dragging")
                return None
    ...
    def select_and_process_code_block_with_ocr(self, search_text, input_text, target_lines=None):
        """
        Use OCR to find the search_text, select lines using mouse dragging, and process with LLM.
        ...
        """
        try:
            # First detect the code block with traditional methods
            ...
            # Get the first line of the code block as search text for OCR
            lines = block_text.splitlines()
            key_line = ""

            # Use the first line of the block that has content
            for line in lines:
                if line.strip():
                    key_line = line.strip()
                    break
            ...
            if not key_line:
                self.log("Could not find a suitable line in code block for OCR")
                return False

            self.log(f"Using key line for OCR selection: '{key_line}'")

            # Use our mouse-dragging based selection
            ...
            if not selected_text:
                ...
                if not selected_text:
                    self.log(f"OCR couldn't find text matching '{key_line}'")
                    return False
    ...
    def select_code_block_with_key_line(self, key_line, total_lines):
        """
        Select a code block by first finding the key line with OCR and triple-clicking it.
        This allows us to position at the exact line that determines the code block.
        ...
        try:
            ...
            # Get position of the key line
            start_x = match['line_start_x']
            start_y = match['line_start_y']

            self.log(f"Found key line at position ({start_x}, {start_y})")

            # Move to the key line position and triple-click to select it
            ...
            # Check if we got something reasonable
            if not selected_line or len(selected_line) < len(key_line) * 0.5:
                self.log(f"Triple-click selected unexpected text: '{selected_line}'")
                self.log("Will proceed with traditional OCR selection")

                # Fall back to traditional OCR selection
            ...
            # Now that we have successfully triple-clicked the line, we can use traditional methods
            # to select the full code block
            self.log(f"Successfully triple-clicked on line: '{selected_line}'")

            # We have the current line selected, now we need to select the determined number of lines
            ...
            line_count = selected_block.count('\n') + 1
            self.log(f"Selected {line_count} lines (target: {total_lines})")
    ...
    def select_and_process_code_block_with_ocr(self, search_text, input_text, target_lines=None):
        ...
        try:
            ...
            # First try our improved OCR-based selection
            selected_text = self.select_code_block_with_key_line(
                key_line,
                lines_in_block
            )
    ...
    def select_and_process_code_block_with_ocr(self, search_text, input_text, target_lines=None):
        ...
        try:
            ...
            # Always use the first line of the detected block for OCR
            lines = block_text.splitlines()
            if not lines:
                self.log("Detected empty code block")
                return False

            # Get the first non-empty line to use as search text for OCR
            ocr_search_text = ""
            for line in lines:
                if line.strip():
                    ocr_search_text = line.strip()
                    break

            if not ocr_search_text:
                self.log("Could not find suitable text in code block for OCR")
                return False

            self.log(f"Using first line of block for OCR search: '{ocr_search_text}'")

            # Use OCR to locate and select the text starting from the first line
            # This completely relies on OCR rather than keyboard navigation
            selected_text, match = self.ocr_selector.select_code_block_with_ocr(
                ocr_search_text,
                lines_in_block
            )
            ...
            if not selected_text:
                self.log(f"OCR couldn't find text matching '{ocr_search_text}'")
                return False

            self.log(f"OCR successfully selected {len(selected_text.splitlines())} lines")
            ...
            # Paste the processed text (selection is already active from OCR)
            pyperclip.copy(processed_text)
            time.sleep(0.2)
            pyautogui.hotkey('ctrl', 'v')
            time.sleep(CONFIG['COPY_PASTE_DELAY'])
