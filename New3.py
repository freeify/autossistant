...
if not hasattr(pyautogui, 'tripleClick'):
    def triple_click():
    ...
    # Add the function to pyautogui module
    pyautogui.tripleClick = triple_click
# Platform detection
...
class OCRTextSelector:
    ...
    def log(self, message):
        ...
        # Only print message if debug mode is active
        if self.debug:
            print(message)

    def clean_text(self, text):
        """Clean text for better matching."""
        # Remove multiple whitespaces and trim leading/trailing spaces
        return re.sub(r'\s+', ' ', text).strip()

    def similarity_score(self, text1, text2):
        """Calculate similarity between two strings."""
        # Use SequenceMatcher to compute similarity ratio between 0 and 1
...
class CodeProcessor:
    ...
    def get_text_and_line_number(self, x, y):
        ...
        # Store the current line for OCR search later
        if current_line and current_line.strip():
            state.current_line_text = current_line.strip()
            self.log(f"Stored current line for OCR search: '{state.current_line_text}'")
    ...
    def preview_code_block(self, x, y):
        ...
        try:
            ...
            # Also store the specific line that determines the code block for later selection
            state.code_block_key_line = lines[start].strip()
            ...
            self.log(block_text)
            self.log("-" * 40)
            self.log(f"Key line for selection: '{state.code_block_key_line}'")
    ...
    # Modify the CodeProcessor class to include a method to capture the selected line
    # Modify the CodeProcessor class to include a method to capture the selected line
    def get_selected_line(self):
        """Get the currently selected line of text (if any)."""
        # Try to copy any currently selected text
        pyautogui.hotkey('ctrl', 'c')
        time.sleep(CONFIG['COPY_PASTE_DELAY'])
        selected_text = pyperclip.paste().strip()

        if selected_text:
            self.log(f"User has selected text: '{selected_text[:50]}...' ({len(selected_text)} chars)")
            return selected_text
        else:
            self.log("No text currently selected")
            return None

    # Now replace the select_code_block_with_key_line method in the CodeProcessor class
    def select_code_block_with_key_line(self, key_line, total_lines):
        """
        Select a code block by first finding the key line with OCR, triple-clicking it,
        determining the exact first line of the code block, then precisely navigating to it.

        Args:
            key_line: The line that determines the reference point (already selected by user)
            total_lines: The estimated total number of lines in the block
        """
        try:
            # Store original clipboard content to restore later
            original_clipboard = pyperclip.paste()

            # The key line is the line the user has already selected/copied
            self.log(f"Using selected key line: '{key_line[:50]}...'")

            # Use OCR to find the key line on screen
            match, estimated_line_height = self.ocr_selector.find_text_on_screen(key_line)
            if not match:
                self.log("Could not find key line using OCR")
                return None

            # Get position of the key line - specifically the beginning of the line
            key_line_x = match['line_start_x']
            key_line_y = match['line_start_y']
            self.log(f"Found key line at position ({key_line_x}, {key_line_y})")

            # Move to the beginning of the line and triple-click to select the whole line
            pyautogui.moveTo(key_line_x, key_line_y, duration=0.1)
            time.sleep(CONFIG['CLICK_DELAY'])

            # Perform the triple click to select the entire line
            pyautogui.tripleClick()
            time.sleep(0.2)  # Slightly longer delay to ensure triple click completes

            # Verify we selected the right line by copying it
            pyautogui.hotkey('ctrl', 'c')
            selected_line = pyperclip.paste().strip()

            # Log what was selected for debugging
            self.log(f"Triple-click selected text: '{selected_line[:50]}...'")

            # If we didn't get the right line, try a different approach
            if not selected_line or not key_line.strip() in selected_line:
                self.log("Triple-click didn't select expected line, trying alternative approach")

                # Try clicking once first to position cursor, then triple-click
                pyautogui.click(key_line_x, key_line_y)
                time.sleep(0.1)
                pyautogui.tripleClick()
                time.sleep(0.2)
                ...
                pyautogui.hotkey('ctrl', 'c')
                selected_line = pyperclip.paste().strip()
                self.log(f"Second attempt selected: '{selected_line[:50]}...'")

                # If still not successful, fall back to traditional selection
                if not selected_line or not key_line.strip() in selected_line:
                    self.log("Falling back to traditional OCR selection")
                    selected_text, _ = self.ocr_selector.select_code_block_with_ocr(
                        key_line,
                        total_lines
                    )
                    return selected_text

            # After triple-clicking, press Escape to clear the selection and position the cursor at that line
            pyautogui.press('escape')
            time.sleep(0.1)

            # Now, get all text to determine the code block structure
            pyautogui.hotkey('ctrl', 'a')  # Select all
            time.sleep(CONFIG['COPY_PASTE_DELAY'])
            pyautogui.hotkey('ctrl', 'c')  # Copy
            full_text = pyperclip.paste()

            # Clear selection and return to the line we were at

            pyautogui.click()
            time.sleep(0.1)

            # Determine the line number in the full text
            lines = full_text.splitlines()
            cursor_line = None

            for i, line in enumerate(lines):
                if line.strip() == selected_line.strip():
                    cursor_line = i
                    break

            if cursor_line is None:
                self.log("Could not find the selected line in the full text")
                return None

            # Detect language and find block boundaries
            language = detect_language(full_text)
            start_line, end_line, block_type = find_block_boundaries(lines, cursor_line, language)

            self.log(f"Detected code block: lines {start_line}-{end_line} ({language}, {block_type})")

            # Get the exact first line of the code block
            first_line_text = lines[start_line].strip()
            self.log(f"First line of code block: '{first_line_text[:50]}...'")

            # Always try OCR first for highest precision
            first_line_match, _ = self.ocr_selector.find_text_on_screen(first_line_text)

            if first_line_match:
                # Successfully found first line with OCR
                first_line_x = first_line_match['line_start_x']
                first_line_y = first_line_match['line_start_y']

                self.log(f"Found exact first line at position ({first_line_x}, {first_line_y})")

                # Move to the beginning of the first line and click to position cursor
                pyautogui.moveTo(first_line_x, first_line_y, duration=0.1)
                pyautogui.click()
                time.sleep(0.1)
                ...
                # Calculate how many lines to select down
                lines_in_block = end_line - start_line + 1
                self.log(f"Block contains {lines_in_block} lines")

                # Start selection
                pyautogui.keyDown('shift')

                # Move down to select all lines in the block
                ...
                # OCR failed to find first line, try relative position calculation
                self.log("Could not find first line with OCR, trying to calculate position")

                # Calculate the line difference between cursor position and start of block
                line_diff = start_line - cursor_line

                if line_diff == 0:
                    # We're already at the first line, just position cursor at the beginning
                    pyautogui.press('home')
                    time.sleep(0.05)
                elif line_diff < 0:
                    # We need to go up |line_diff| lines
                    self.log(f"Need to go up {abs(line_diff)} lines")
                    ...
                    # Unexpected case, first line should be at or above cursor
                    self.log(f"Warning: First line is below cursor position, unexpected case")
                    # Try to find the beginning of the file and navigate from there
                    pyautogui.keyDown('ctrl')
                    pyautogui.press('home')  # Go to beginning of file
                    pyautogui.keyUp('ctrl')
                    time.sleep(0.1)

                    # Press down to get to the start_line
                ...
                # Now we should be at the first line of the block
                # Start selection
                pyautogui.keyDown('shift')

                # Calculate how many lines to select (end_line - start_line + 1)
                lines_to_select = end_line - start_line + 1

                # Navigate down to select all required lines
                for _ in range(lines_to_select - 1):
                    pyautogui.press('down')
                    time.sleep(0.01)
            ...
            pyautogui.hotkey('ctrl', 'c')
            selected_block = pyperclip.paste()

            # Log statistics
            self.log(f"Selected code block has {len(selected_block.splitlines())} lines")

            # Verify we have the correct starting line by checking the first line of selected_block
            selected_lines = selected_block.splitlines()
            if selected_lines and selected_lines[0].strip() != first_line_text.strip():
                self.log(f"Warning: Selected block doesn't start with expected first line")
                self.log(f"Expected: '{first_line_text[:50]}...'")
                self.log(f"Got: '{selected_lines[0][:50]}...'")

                # Try one more approach: direct OCR search with triple-click + shift+down
                if first_line_match:
                    self.log("Trying direct OCR selection with triple-click approach")
                    # Move to the first line position again
                    pyautogui.moveTo(first_line_match['line_start_x'], first_line_match['line_start_y'], duration=0.1)
                    # Triple-click to select the first line
                    pyautogui.tripleClick()
                    time.sleep(0.1)

                    # Start extending selection down
                    pyautogui.keyDown('shift')
                    for _ in range(lines_in_block - 1):
                        pyautogui.press('down')
                        time.sleep(0.01)
                    ...
                    selected_lines = selected_block.splitlines()
                    if selected_lines and selected_lines[0].strip() == first_line_text.strip():
                        self.log("Successfully selected correct block with triple-click approach")
                    else:
                        self.log("Still couldn't select correct block, using best attempt")
            ...
            self.log(f"Error in select_code_block_with_key_line: {e}")
            traceback.print_exc()
    ...
    # Now update select_and_process_code_block_with_ocr to use this new method
    def select_and_process_code_block_with_ocr(self, search_text, input_text, target_lines=None):
        ...
        """
        try:
            # First, get the currently selected line from the user
            # Save original clipboard content
            original_clipboard = pyperclip.paste()

            # Get the selected text (line) from the user
            pyautogui.hotkey('ctrl', 'c')
            time.sleep(CONFIG['COPY_PASTE_DELAY'])
            selected_line = pyperclip.paste().strip()

            if not selected_line:
                self.log("No text is currently selected by the user")
                # Try to put the cursor back
                pyautogui.click()
                return False

            self.log(f"User has selected line: '{selected_line[:50]}...'")

            # Now detect the code block to determine line count
            ...
            # Use the user-selected line as our key line for OCR selection
            self.log(f"Using user-selected line for OCR selection: '{selected_line[:50]}...'")

            # Try our improved OCR-based selection with the user's selected line
            selected_text = self.select_code_block_with_key_line(
                selected_line,
                lines_in_block
            )

            # Fall back to traditional OCR selection if needed
            if not selected_text:
                ...
                if not selected_text:
                    self.log(f"OCR couldn't find text matching the selected line")
                    # Restore original clipboard
                    pyperclip.copy(original_clipboard)
                    return False

            self.log(f"Successfully selected {len(selected_text.splitlines())} lines")
            ...
            traceback.print_exc()
            # Try to restore original clipboard in case of error
            try:
                pyperclip.copy(original_clipboard)
            except:
                pass
